# bot/handlers/tabs/buy_stars.py - ИСПРАВЛЕННАЯ ВЕРСИЯ
...
async def buy_stars_package(update: Update, context: ContextTypes.DEFAULT_TYPE, query, uid: int, package_id: str):
    ...
    if package:
        ...
        text = get_text(uid, "stars_bought").format(
            name=package['name'],
            stars=stars,
            price=price
        )
        
        await query.message.edit_text(text, reply_markup=tab_kb(uid))
    else:
        await query.message.edit_text(get_text(uid, "package_not_found"), reply_markup=tab_kb(uid))