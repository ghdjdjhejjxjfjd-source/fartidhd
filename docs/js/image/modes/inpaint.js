    ...
    container.innerHTML = `
        ...
        <div class="result-area" id="resultArea">
            ...
            <div class="loading-overlay hidden" id="loadingOverlay">
                ...
                <div class="loading-text">Удаление объекта... <span id="loadingTimer">0с</span></div>
            </div>
        </div>
...
async function generateImage() {
    ...
    if (!prompt) {
        showToast('Опишите что нужно удалить', 'error');
        return;
    }
...
function saveToGallery() {
    ...
    const prompt = document.getElementById('promptInput')?.value || 'Удаление объекта';
...
function downloadImage() {
    ...
    const filename = originalFileName 
        ? `${originalFileName}_inpainted.jpg` 
        : `inpainted_${Date.now()}.jpg`;
