...
export function createChatController({ chatEl, inputEl, sendBtnEl }) {
  ...
  let searchMode = false;  // По умолчанию выключен
  console.log("🔍 searchMode initialized:", searchMode);
  ...
  async function send() {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        // 👇 Передаём searchMode в askAI
        const answer = await askAI(fullPrompt, searchMode);
